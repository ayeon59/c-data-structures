...
int hasGreatGrandchild(BTNode *node)
{
    ...
    int countL = hasGreatGrandchild(node->left);
    int countR = hasGreatGrandchild(node->right);
    if (countL>=2 ||countR>=2) printf("%d ",node->item);
    return ((countL>countR ? countL : countR) + 1);
}
